// build by:
// gcc sdl_graphic_tiles.c -o sdl_graphic_tiles -lSDL2 -lm -Wall -Werror
...
void render_logo(SDL_Renderer *renderer, SDL_Texture *texture, uint16_t *bit16_logo) {
    ...
    for (int row = 0; row < LOGO_HEIGHT; row++) {
        ...
        for (int col = 0; col < LOGO_WIDTH; col++) {
            ...
            // Asigns a single 4bit group based on row.. Shifts >> Left N amount, grabbing 4 bits
            uint32_t bitrow_val = (pixels_storage >> bitgroup_loc[row]) & 0xf;
            // Grabs only 1 bit, shifting the bits between 3-0.
...
int main() {
    ...
    // BACKGROUND IS 256 X 256.     (32x32 Tiles)

    // VIEWPORT (WINDOW)
    // Max is 160x144               (20x18 Tiles.)
