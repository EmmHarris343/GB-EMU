...
int cart_mbc_log_init(const char *filename) {
    cart_mbc_log_file = fopen(filename, "w");
    if (!cart_mbc_log_file) {
        printf("[ERROR] Failed to open Trace log file!\n");
        return -1;
    }
    return 0;
...
void cart_mbc_log_close() {
    if (cart_mbc_log_file) {
        fclose(cart_mbc_log_file);
        cart_mbc_log_file = NULL;
    }
}
...
void logging_log(const char *format, ...) {
    ...
    // Format mostly looks like:
    // STEP PC OPC A F B C D E H L SP IME IE IF
...
void logging_cart_mbc_log(const char *format, ...) {
    if (!cart_mbc_log_file) return;
    va_list args;
    va_start(args, format);
    vfprintf(cart_mbc_log_file, format, args);
    va_end(args);

    fflush(cart_mbc_log_file);
}
